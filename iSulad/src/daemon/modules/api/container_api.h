...
typedef struct _container_t_ {
    pthread_mutex_t mutex;  //容器互斥锁
    bool init_mutex;  //容器互斥锁初始化标签
    pthread_cond_t wait_stop_con;  //容器等待容器停止的条件变量
    bool init_wait_stop_con;  //条件变量初始化标签
    pthread_cond_t wait_rm_con;  //容器等待remove的条件变量
    bool init_wait_rm_con; 
    uint64_t refcnt; //引用计数 用于gc
    char *runtime;
    char *root_path;
    char *state_path;
    char *image_id;
