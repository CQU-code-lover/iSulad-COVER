...
//为gc_save_containers_config服务 用于将容器gc的配置信息保存到文件中
static int save_gc_config(const char *json_gc_config)
{
    ...
    //将rootpath与GCCONFIGJSON使用“/”拼接并且复制到filename中，filename是配置文件的全路径
    ...
    //根据filename创建对应的配置文件
    //CONFUF_FILE_MODE为640（表示创建的文件在操作系统中的权限，所有者可读写，用户组中可读，其他用户无权限）
    ...
    //向配置文件写入信息 配置信息保存在json_gc_config字符串中
...
static int gc_containers_to_disk()
{
    ...
    if (size != 0) {
        ...
        //使用链表迭代器将链表的节点中的elem存放在conts数组中
    ...
    //将conts数组保存在saves中 saves即是要回收的数据的结构
